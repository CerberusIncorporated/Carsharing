package by.yukhnevich.carsharing.carsharing.model.dao.impl;
...
public class CarDaoImpl implements CarDao {
    ...
    private static final String UPDATE_CAR_QUERY = "UPDATE cars SET brand=?, model=?, color=?, seats=?, gearbox=?, manufactured_year=?, engine_type=?, class=?, price_per_day=?, image_path=? WHERE id=?;";
    private static final String DELETE_BY_ID_QUERY = "DELETE FROM cars WHERE id=?;";
    private static final String ADD_CAR_QUERY = "INSERT INTO cars (brand, model, color, seats, gearbox, manufactured_year, engine_type, price_per_day, vin, plate, class, image_path) " +
            "VALUE (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<Car> executeForManyResults(PreparedStatement statement) throws SQLException {
        ...
        while (resultSet.next()) {
            ...
            Car car = new Car(id, brand, model, color, seats, gearboxType, year,
    ...
    public Optional<Car> executeForSingleResult(PreparedStatement statement) throws SQLException {
        ...
        if (resultSet.next()) {
            ...
            car = Optional.of(new Car(id, brand, model, color, seats, gearboxType, year,
                    engineType, pricePerDay, vin, plate, carClass, imagePath));
        }
